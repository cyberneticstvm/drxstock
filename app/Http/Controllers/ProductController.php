    ...
    function trackFetch(Request $request)
    {
        ...
        try {
            ...
            $products = Product::withTrashed()->when($request->axis != '', function ($q) use ($axis) {
                ...
                return $q->where('sph', $spherical)->orWhere('sph', "CAST($spherical AS DECIMAL(4,2))+CAST($cylinder AS DECIMAL(4,2))")->where('cyl', $cylinder)->orWhere('cyl', "CAST(0-$cylinder AS DECIMAL(4,2))");

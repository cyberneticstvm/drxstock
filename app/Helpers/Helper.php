...
function getInventory($product, $editQty)
{
    ...
    $stock = DB::select("SELECT tbl2.product_id, tbl2.pcode, tbl2.product_name, tbl2.purchasedQty, tbl2.damagedQty, SUM(IFNULL(sd.qty, 0)) AS soldQty, ((tbl2.purchasedQty + ?)-(SUM(IFNULL(sd.qty, 0)) + tbl2.damagedQty)) AS balanceQty FROM (SELECT tbl1.*, IFNULL(SUM(CASE WHEN d.deleted_at IS NULL THEN d.qty ELSE 0 END), 0) AS damagedQty FROM (SELECT p.id AS product_id, p.code AS pcode, p.name AS product_name, SUM(IFNULL(pd.qty, 0)) AS purchasedQty FROM purchase_details pd LEFT JOIN products p ON pd.product_id = p.id WHERE IF(? > 0, pd.product_id = ?, 1) AND pd.is_return IS NULL AND pd.deleted_at IS NULL GROUP BY p.id, p.name, p.code) AS tbl1 LEFT JOIN damages d ON d.product_id = tbl1.product_id GROUP BY tbl1.product_id, tbl1.pcode, tbl1.product_name, tbl1.purchasedQty) AS tbl2 LEFT JOIN sales_details sd ON sd.product_id = tbl2.product_id WHERE sd.is_return IS NULL AND sd.deleted_at IS NULL GROUP BY tbl2.product_id, tbl2.pcode, tbl2.product_name, tbl2.purchasedQty, tbl2.damagedQty", [$editQty, $product, $product]);
